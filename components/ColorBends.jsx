...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // Defer Three.js init to avoid blocking initial render
    let cancelled = false;
    let cleanup = null;
    ...
    // Initialize colors array immediately with passed colors prop
    const uColorsArray = Array.from({ length: MAX_COLORS }, () => new THREE.Vector3(0, 0, 0));
    const colorArray = (colors || []).filter(Boolean).slice(0, MAX_COLORS).map(toVec3);
    for (let i = 0; i < MAX_COLORS; i++) {
      if (i < colorArray.length) uColorsArray[i].copy(colorArray[i]);
    }
    ...
    if ('requestIdleCallback' in window) {
      const id = requestIdleCallback(initThree, { timeout: 100 });
      return () => {
        cancelled = true;
      ...
      const id = setTimeout(initThree, 50);
      return () => {
        cancelled = true;
        clearTimeout(id);
        if (cleanup) cleanup();
