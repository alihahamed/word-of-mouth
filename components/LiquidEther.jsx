...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const lowPower =
      ...
      (performanceMode === 'auto' && (isMacOS || isSafari));

    const perf = {
      lowPower,
      maxDpr: lowPower ? 1.25 : 2,
      antialias: !lowPower,
      resolutionScale: lowPower ? 0.85 : 1,
      iterationsScale: lowPower ? 0.55 : 1,
      effectiveFps: lowPower ? Math.min(targetFps, 50) : targetFps
    };

    const buildSimOptions = currentResolution => ({
      mouse_force: mouseForce,
      cursor_size: cursorSize,
      isViscous,
      viscous,
      iterations_viscous: Math.max(8, Math.round(iterationsViscous * perf.iterationsScale)),
      iterations_poisson: Math.max(8, Math.round(iterationsPoisson * perf.iterationsScale)),
      dt,
      BFECC: lowPower ? false : BFECC,
      resolution: Math.max(0.2, Math.min(1, currentResolution * perf.resolutionScale)),
      isBounce
    });
    ...
    class CommonClass {
      ...
      init(container) {
        ...
        this.pixelRatio = Math.min(window.devicePixelRatio || 1, perf.maxDpr);
        this.resize();
...
`;
    ...
    class WebGLManager {
      constructor(props) {
        ...
        this.running = false;
        this.targetFrameTime = 1000 / Math.max(24, perf.effectiveFps);
        this._lastFrameTs = 0;
      }
      init() {
      ...
      loop(ts = performance.now()) {
        ...
        if (ts - this._lastFrameTs >= this.targetFrameTime) {
    ...
    const applyOptionsFromProps = () => {
      ...
      const nextOptions = buildSimOptions(resolution);
      Object.assign(sim.options, nextOptions);
      if (nextOptions.resolution !== prevRes) {
        sim.resize();
      }
    };
  ...
  useEffect(() => {
    ...
    const lowPower =
      ...
      prefersReducedMotion ||
      (performanceMode === 'auto' && (isMacOS || isSafari));
    const resolutionScale = lowPower ? 0.85 : 1;
    const iterationsScale = lowPower ? 0.55 : 1;
    ...
    if (!sim) return;
    const prevRes = sim.options.resolution;
    const nextResolution = Math.max(0.2, Math.min(1, resolution * resolutionScale));
    Object.assign(sim.options, {
      mouse_force: mouseForce,
      cursor_size: cursorSize,
      isViscous,
      viscous,
      iterations_viscous: Math.max(8, Math.round(iterationsViscous * iterationsScale)),
      iterations_poisson: Math.max(8, Math.round(iterationsPoisson * iterationsScale)),
      dt,
      BFECC: lowPower ? false : BFECC,
